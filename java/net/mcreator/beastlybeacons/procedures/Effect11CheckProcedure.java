...
public class Effect11CheckProcedure {
	public static boolean execute(LevelAccessor world, double x, double y, double z) {
		double count = 0;
		if (MidLayerCheckProcedure.execute(world, x, y, z) == true && BottomLayerCheckProcedure.execute(world, x, y, z) == true) {
			count = TopLayerCountProcedure.execute(world, x, y, z) + MidLayerCountProcedure.execute(world, x, y, z) + BottomLayerCountProcedure.execute(world, x, y, z);
			if (new Object() {
			...
			}.getEnergyStored(world, BlockPos.containing(x, y, z)) > 0 || !world.getLevelData().getGameRules().getBoolean(BeastlyBeaconsModGameRules.REQUIRE_BEACON_EXP)) && new Object() {
				public int getAmount(LevelAccessor world, BlockPos pos, int slotid) {
					if (world instanceof ILevelExtension _ext) {
						IItemHandler _itemHandler = _ext.getCapability(Capabilities.ItemHandler.BLOCK, pos, null);
						if (_itemHandler != null)
			...
			}.getAmount(world, BlockPos.containing(x, y, z), 11) == 1 && count >= 81) {
				return true;
			}
		}
		return false;
