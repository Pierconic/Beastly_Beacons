...
public class EvilSentinelTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!(entity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(BeastlyBeaconsModMobEffects.BENEVOLENCE))
				|| !(!world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 32, 32, 32), e -> true).isEmpty()) && Math.random() < 0.001) {
		...
		if ((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) instanceof LivingEntity && !(entity instanceof LivingEntity _livEnt7 && _livEnt7.hasEffect(MobEffects.UNLUCK))) {
			...
			entity.setDeltaMovement(new Vec3(((x_distance / magnitude) * (-0.2)), ((y_distance / magnitude) * (-0.2)), ((z_distance / magnitude) * (-0.2))));
		}
		found = false;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (found == false && entityiterator instanceof Player && entityiterator instanceof LivingEntity _livEnt19 && _livEnt19.hasEffect(BeastlyBeaconsModMobEffects.MALEVOLENCE)) {
					if (Math.random() < 0.3) {
						found = true;
